...
| THR-05 | Endpoint quản trị (`/api/users`, `/api/roles`, `/api/permissions`) | Tài khoản Student/Staff tự cấp thêm vai trò cho mình → leo thang lên Admin | **Cao** | Cả ba controller gắn `[Authorize(Roles = "Admin")]` ở mức class | Ca C8 (Staff) → 403; ca C9 (Admin) → 200 |
